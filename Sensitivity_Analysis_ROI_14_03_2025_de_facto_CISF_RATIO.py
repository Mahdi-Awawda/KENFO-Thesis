...
# --------------------------
# Create a sensitivity analysis plot for ROI ratio relative to the base inflation rate (1.60%)
# --------------------------
...
# Plot each scenario as a factor relative to the base (1.60%)
for scenario in inflation_scenarios:
    if scenario == "1.60":
...
# Adjust y-axis ticks: set ticks at an interval of 0.1 based on the data range
all_ratios = []
for scenario in inflation_scenarios:
    if scenario == "1.60":
...
plt.legend(loc='upper left', bbox_to_anchor=(0.72, 0.68), borderaxespad=0.)

# Generate a unique filename for the sensitivity plot
sensitivity_plot_path = get_next_filename("sensitivity_analysis_de_facto_CISF_ratio", "png", sensitivity_dir)
